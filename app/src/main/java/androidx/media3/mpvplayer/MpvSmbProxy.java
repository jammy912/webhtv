...
/**
 * Serves {@code smb://} sources to MPV over loopback HTTP.
 *
 * <p>MPV's native stream layer is built without SMB support (the pinned curl
 * override passes {@code --disable-smb}), so {@code smb://} URLs fail with
 * "No protocol handler found". Media3 already ships {@link SmbDataSource},
 * which reads shares through smbj at arbitrary file offsets, so this proxy
 * bridges the two: it reads via {@link SmbDataSource} and republishes the
 * bytes as a range-capable HTTP resource that MPV can open.
 */
public final class MpvSmbProxy extends NanoHTTPD {

    private static final String TAG = "mpv-smb-proxy";
    ...
    /**
     * Registers {@code url} and returns the loopback HTTP URL MPV should open.
     */
